    ...
    $.each(question.answers, function(answer_index, answer) {
      ...
      if (correct) {
        ...
        opts = $.extend(opts, {
          title: "Que mal",
          text: (
            "No es la respuesta correcta<br/><br/>" +
...
function resultsText(state) {
  ...
  switch (true) {
    case (ratio === 1):
      text = "Wow&mdash;Puntuacion Perfecta¡";
      break;
    case (ratio > 0.9):
      text = "Increible, casi todas estan correctas";
      break;
    case (ratio > 0.60):
      text = "Bien, pasaste";
      break;
    case (ratio > 0.5):
      text = "Obtuviste la mitad&hellip;";
      break;
    case (ratio < 0.5 && ratio !== 0):
      text = "Mejor suerte para la proxima";
      break;
    case (ratio === 0):
      text = "Solo con poner tu nombre tenias 10";
      break;
  }
  return text;
